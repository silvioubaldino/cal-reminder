...
| RF-17 | Report anonymous Telemetry | Should | The Distributed Build reports to the Project Service three counts and nothing else: Reminder animations played; one "used today" event per calendar day, raised whenever the app is running on an awake Mac — on launch, on wake from sleep, on an animation, or on its own timer; and one event per installation, marked as a first install or an update. Each is labelled with the app and macOS version. A report carries **no identifier of any kind** and no Event, Calendar or Account content. The menu bar states that it reports and offers a switch to stop it; the first launch says so before the first report leaves the Mac. A **Source Build** reports nothing (RNF-13) |
| RF-18 | Send a crash report with consent | Should | After an abnormal termination, the app finds the crash report macOS itself wrote for its process, shows the user what it contains, and sends it only if the user agrees — one decision per report, never a standing permission, and with no identifier attached. A **Source Build** never offers it. **No design yet**: AYD-012 deliberately leaves crash reporting out, since a crash is a document rather than a count and brings its own questions (symbolication, grouping, retention); it gets its own AYD |
...
| RNF-13 | Observability | The project can tell how the app is doing in the field, without collecting anything about anyone | A dashboard shows Reminder animations played, how many Installs were used each day and on which app and macOS version, and how many installations happened split into first installs and updates — with at least 12 months of history. Reports carry **no identifier**, so no Install, Mac or person can be followed across reports, and no IP is retained. Daily counts are read one day at a time: summed over a longer window they are Install-days, not distinct Installs, and every count is a lower bound because Telemetry can be switched off — the dashboard says both. A build with **no** Project Service configuration makes **no** request to any project-owned server — not Telemetry, not the update check — and that is covered by an automated test (AYD-012) |
