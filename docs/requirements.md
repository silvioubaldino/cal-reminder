...
| RF-17 | Report anonymous Telemetry | Should | The Distributed Build reports to the Project Service how many Reminder animations it played, and its own current state — app version, macOS version, number of connected Accounts, number of selected Calendars — carrying a random Install identifier and never any Event, Calendar or Account content. The menu bar states that it reports and offers a switch to stop it; the first launch says so before the first report leaves the Mac. A **Source Build** reports nothing (RNF-13) |
| RF-18 | Send a crash report with consent | Should | After an abnormal termination, the app finds the crash report macOS itself wrote for its process, shows the user what it contains, and sends it to the Project Service only if the user agrees — one decision per report, never a standing permission. A **Source Build** never offers it (RNF-13) |
...
| RNF-13 | Observability | The project can tell how the app is doing in the field, without collecting personal data | A dashboard shows active Installs (1/7/30 days), new Installs, the spread of app versions, the Accounts and Calendars connected across the fleet, and Reminder animations played, with at least 12 months of history. A report carries a random Install identifier and nothing personal: no email, no Event, Calendar or Account content, and no retained IP. A build with **no** Project Service configuration makes **no** request to any project-owned server — not Telemetry, not the update check, not an entitlement check — and that is covered by an automated test (AYD-012) |
...
observability it exists for — RF-17, RF-18, RNF-13). AYD-009 depends on the signing configuration
AYD-010 defines; AYD-012 depends on the build-time configuration TDR-007 introduced.
...
| Install | _One copy of the app running on one Mac, identified by a random identifier generated on first launch and kept in the Keychain. It is the unit the project counts (RNF-13) — not a person, and it carries nothing personal._ | "user", "device", "seat" |
| Project Service | _The project-owned backend a Distributed Build talks to: it receives Telemetry (RF-17) and crash reports (RF-18), and later issues subscription entitlements. A Source Build never contacts it._ | "API", "server", "backend" |
| Telemetry | _What a Distributed Build reports to the Project Service: the Reminder animations it played, and the Install's current state (versions, connected Accounts, selected Calendars). Never Event, Calendar or Account content._ | "analytics", "tracking", "metrics" |
