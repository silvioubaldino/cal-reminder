...
| RNF-13 | Observability | The project can tell how the app is doing in the field, without collecting anything about anyone | A dashboard shows Reminder animations played, how many Installs were used each day and on which app and macOS version, how many updates were installed, and crashes by version — with at least 12 months of history. Reports carry **no identifier**, so no Install, Mac or person can be followed across reports, and no IP is retained; every count is therefore a lower bound and is labelled as such. A build with **no** Project Service configuration makes **no** request to any project-owned server — not Telemetry, not the update check — and that is covered by an automated test (AYD-012) |
...
| Telemetry | _What a Distributed Build reports to the Project Service: a batch of counter events — Reminder animations played, "used today", "update installed" — labelled with the app and macOS version. No identifier, and never Event, Calendar or Account content._ | "analytics", "tracking", "metrics" |
