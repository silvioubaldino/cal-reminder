...
> allowlisted counter events plus crash reports, and publishes every counter to Cloud Monitoring.
...
2. `POST /v1/crash` stores a crash report and counts it.
3. A name outside the allowlist, or a value outside its range, is dropped without failing the
   batch or the request.
4. Every metric is published to Cloud Monitoring per TDR-008 and visible on a Grafana dashboard.
5. The service cannot run up a bill under flood.

Out of scope: the app-side client (SPEC-023); crash symbolication and retention (SPEC-024);
...
Scenario: A batch increments one counter per event
  ...
  When POST /v1/events arrives with planes_flown 3 and daily_active 1
...
Scenario: A crash report is stored and counted
  ...
  Then the response is 202
  And the report is stored in the crash bucket
...
- **Answer first, record after**: `/v1/events` and `/v1/crash` answer `202` and do their work on a
...
2. Metrics package: meter provider per TDR-008 and the four counters from AYD-012's table.
3. The allowlist table and its validator.
4. `/v1/events` with the key middleware, the size and batch limits, and the validator.
5. `/v1/crash`: write the raw report to Cloud Storage under `crash/{yyyy-mm-dd}/{uuid}.ips`,
   increment the counter.
6. Deploy workflow, triggered only on `service/**`: build, deploy to Cloud Run with
   `--max-instances` set, Workload Identity Federation instead of a service-account key.
7. Log exclusion for client IPs; a per-IP rate limit in front of the handlers.
8. Grafana dashboard on the Cloud Monitoring datasource: airplanes per day, Installs used per day
   by version, updates installed, crashes — with a note that every count is a lower bound and that
   `daily_active_total` is not unique users.
9. A budget alert on the project.
...
## Tests
- **Acceptance:** one test per Gherkin scenario, with the metric recorder and the crash storage
  faked at the boundary.
- **Unit:** the allowlist validator — unknown name, value below and above each range, bad
  `appVersion` pattern, label set enforcement.
...
- [ ] Dashboard states that counts are lower bounds and not unique users
